...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channel=3, width=1,
                 ...
                 head_type='pass', layer4_dilation=1, pretrained_weight=None):
        ...
        if pretrained_weight is not None:
            ...
            state_dict = {k: v for k, v in state_dict.items() if not k.endswith('running_mean')}
            state_dict = {k: v for k, v in state_dict.items() if not k.endswith('running_var')}
            state_dict = {k: v for k, v in state_dict.items() if not k.endswith('num_batches_tracked')}
            msg = self.load_state_dict(state_dict, strict=False)
            print('\nPretrained weights found at {} and loaded with msg: {}'.format(pretrained_weight, msg))
    ...
    def get_embeddings(self, x):
        ...
        for t in tmp:
            B, d = t.shape[:2]
            t = t.reshape(B, d, -1).permute(0,2,1)
            cls_embeddings.append(t.mean(dim = 1))
            res.append(t)
        
        return cls_embeddings, res
